...
const nextQuestion = () => {
    if (currentQuestionIndex.value < questions.value.length - 1) {
        ...
        nextTick(() => {
            const el = assessmentCard.value?.$el || assessmentCard.value
        ...
        <Transition name="fade" mode="out-in">
            <div v-if="sessionState === 'idle'" class="flex flex-col gap-8 w-full">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <UCard variant="soft" :ui="{ body: 'relative flex flex-col gap-8' }">
                        <div class="absolute inset-0 overflow-hidden rounded-xl pointer-events-none">
                            <div class="absolute -top-16 -right-16 w-64 h-64 bg-green-500/10 rounded-full blur-[64px]">
                            </div>
                        </div>
                    ...
                    <UCard variant="soft" :ui="{ body: 'relative p-8 flex flex-col gap-8' }">
                        <div class="absolute inset-0 overflow-hidden rounded-xl pointer-events-none">
                            <div class="absolute -top-16 -right-16 w-64 h-64 bg-orange-500/10 rounded-full blur-[64px]">
                            </div>
                        </div>
                        <div class="relative z-10 flex items-center gap-2">
                            <UIcon name="i-lucide-alert-circle" class="text-orange-500" />
                            <h2 class="text-sm font-bold tracking-wider text-orange-500 uppercase">Weak Topics — Focus
                                Areas
                            </h2>
                        </div>
                        <div class="relative z-10 flex flex-col gap-6">
                ...
                <UCard
                    ...
                    <!-- Decorative Glows -->
                    <div
                        class="absolute top-0 left-0 w-1/2 h-full bg-primary/5 rounded-full blur-[100px] -translate-x-1/2">
                    </div>
                    ...
                    <div
                        ...
                        <div class="flex flex-col gap-2">
                            <h3 class="text-xs font-bold tracking-[0.2em] text-primary uppercase">Recommended for you
                            </h3>
                            <p class="text-3xl font-bold tracking-tight leading-tight">
                                Focus on <span
                                    class="text-primary underline decoration-primary/30 underline-offset-8">{{
                    ...
                    <UButton @click="startSession" label="Press Play" icon="i-lucide-play-circle" size="xl"
                        color="primary"
            ...
            <UCard v-else ref="assessmentCard"
                ...
                :ui="{ body: 'p-0 sm:p-0' }">
                <!-- Processing State -->
                <div v-if="sessionState === 'processing'"
                    class="p-8 md:p-16 flex flex-col gap-6 font-mono text-sm text-neutral-500 dark:text-neutral-400 min-h-[400px] justify-center items-start w-full max-w-4xl mx-auto">
                    <div
                    ...
                    <div v-if="activeLines.length < processingLines.length" class="flex gap-2 mt-4 items-center">
                        <span class="w-2.5 h-2.5 bg-neutral-400 dark:bg-neutral-600 rounded-full animate-bounce"
                ...
                <!-- Ready State -->
                <div v-else-if="sessionState === 'ready'"
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 w-full max-w-5xl">
                        <!-- Card 1 -->
                        <div
                            class="bg-white dark:bg-neutral-900 ring-1 ring-neutral-200 dark:ring-neutral-800 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 text-center shadow-sm">
                            <span
                                class="text-[10px] tracking-[0.2em] uppercase text-neutral-500 dark:text-neutral-400 font-semibold">Topic</span>
                        ...
                        <div
                            class="bg-white dark:bg-neutral-900 ring-1 ring-neutral-200 dark:ring-neutral-800 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 text-center shadow-sm">
                            <span
                                class="text-[10px] tracking-[0.2em] uppercase text-neutral-500 dark:text-neutral-400 font-semibold">Module</span>
                            <span class="text-xl font-bold leading-tight text-primary">{{ recommendedLesson?.title
                                ||
                                'Scenario Court' }}</span>
                        </div>
                        <!-- Card 3 -->
                        <div
                            class="bg-white dark:bg-neutral-900 ring-1 ring-neutral-200 dark:ring-neutral-800 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 text-center shadow-sm">
                            <span
                                class="text-[10px] tracking-[0.2em] uppercase text-neutral-500 dark:text-neutral-400 font-semibold">Difficulty</span>
                        ...
                        <div
                            class="bg-white dark:bg-neutral-900 ring-1 ring-neutral-200 dark:ring-neutral-800 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 text-center shadow-sm">
                            <span
                                class="text-[10px] tracking-[0.2em] uppercase text-neutral-500 dark:text-neutral-400 font-semibold">Duration</span>
                            <span class="text-xl font-bold leading-tight text-primary">{{ recommendedLesson?.duration
                                ||
                                '~10 min' }}</span>
                        </div>
                    </div>

                    <UButton @click="beginSessionAction" label="Begin Session" trailing-icon="i-lucide-arrow-right"
                        size="xl" color="primary"
                        class="rounded-full px-16 py-5 uppercase text-sm font-semibold tracking-[0.2em] hover:scale-105 active:scale-95 transition-all duration-300 shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30" />
                </div>

                <!-- Active Session State -->
                <div v-else-if="sessionState === 'active' && currentQuestion"
                    class="p-8 md:p-12 flex flex-col gap-10 animate-fade-in bg-white dark:bg-neutral-900">
                    <!-- Progress Header -->
                    <div
                        ...
                        <div class="flex flex-col gap-1">
                            <span class="text-[10px] tracking-widest uppercase font-bold text-primary">{{
                                recommendedLesson?.title }}</span>
                            <h2 class="text-xl font-bold">Question {{ currentQuestionIndex + 1 }} of {{ questions.length
                            }}</h2>
                        </div>
                        <UButton icon="i-lucide-x" color="neutral" variant="ghost" @click="sessionState = 'idle'"
                            class="rounded-full" />
                    </div>
                    ...
                    <div class="flex flex-col gap-12 w-full max-w-3xl mx-auto">
                            ...
                            <button v-for="option in currentQuestion.options" :key="option.id"
                                @click="selectOption(option.id)" :disabled="!!selectedOptionId" :class="[
                                    ...
                                    selectedOptionId && option.id === currentQuestion.correct
                                        ? 'border-green-500 bg-green-500/5 ring-8 ring-green-500/5'
                                        : '',
                                    // Wrong Selected Answer
                                    selectedOptionId === option.id && option.id !== currentQuestion.correct
                                        ? 'border-red-500 bg-red-500/5 ring-8 ring-red-500/5'
                                        : '',
                                    // Idle State
                                    !selectedOptionId
                                    ...
                                    selectedOptionId && option.id !== currentQuestion.correct && selectedOptionId !== option.id
                                        ? 'opacity-40 border-neutral-100 dark:border-neutral-800 scale-95'
                                        : ''
                                ]">
                                <div :class="[
                                    ...
                                    <UIcon
                                        :name="option.id === 'true' ? 'i-lucide-check-circle-2' : 'i-lucide-x-circle'"
                                        class="w-8 h-8" />
                                </div>
                        ...
                        <div v-else class="grid grid-cols-1 gap-4">
                            <button v-for="option in currentQuestion.options" :key="option.id"
                                ...
                                <UIcon
                                    v-else-if="selectedOptionId === option.id && option.id !== currentQuestion.correct"
                        ...
                        <Transition name="fade">
                            <div v-if="selectedOptionId" ref="reasoningSection"
                                class="flex flex-col gap-8 p-8 md:p-10 rounded-4xl bg-white dark:bg-[#0a0a0a] text-neutral-900 dark:text-white border border-neutral-100 dark:border-neutral-800 shadow-2xl animate-fade-in relative overflow-hidden scroll-mt-24">
                                <div
                                ...
                                <div
                                    class="text-xl md:text-2xl leading-relaxed text-neutral-800 dark:text-neutral-200 relative z-10 italic">
                                    Good choice. But explain it to me — why <span
                                        class="text-primary not-italic font-bold">{{
                                            currentQuestion?.options?.find(o => o.id === selectedOptionId)?.label}}</span>
                                    and not,
                                    say, <span class="text-orange-400/80 not-italic font-bold">{{
                                        currentQuestion?.options?.find(o => o.id !== selectedOptionId)?.label}}</span>?
                                    Those
                                    sound related.
                                </div>
                                ...
                                <div v-if="!isReasoningSubmitted" class="flex justify-start relative z-10">
                                    <UButton label="Submit Reasoning" trailing-icon="i-lucide-arrow-right" size="xl"
                                        color="primary" :loading="isEvaluating" @click="submitReasoning"
                                ...
                                <Transition name="fade">
                                    <div v-if="isReasoningSubmitted" ref="evaluationSection"
                                        class="flex flex-col gap-6 pt-10 mt-2 border-t border-neutral-100 dark:border-neutral-800/50 relative z-10 animate-fade-in scroll-mt-24">
                                        <div
                                            class="flex items-center gap-2 text-[10px] tracking-[0.2em] font-bold uppercase text-primary">
                                            <span>→ Acumen Evaluates</span>
                                        </div>

                                        <div
                                            class="text-lg md:text-xl leading-relaxed text-neutral-700 dark:text-neutral-300 italic">
                                            Close, but be specific. <span class="text-primary not-italic font-bold">{{
                                                currentQuestion?.options?.find(o => o.id ===
                                                    currentQuestion?.correct)?.label
                                            }}</span> happens after thorough analysis — you identify the core
                                            mechanics.
                                            Here, the logic is sound, and it's specifically about the <span
                                                class="text-neutral-800 dark:text-neutral-100 not-italic font-bold">underlying
                                                principles</span>. The trigger matters. Try articulating it that way.
                                        </div>
                                        ...
                                        <div class="flex justify-start pt-4">
                                            <button @click="nextQuestion"
                                                ...
                                                <UIcon name="i-lucide-arrow-right"
                                                    class="w-3.5 h-3.5 transition-transform group-hover:translate-x-2" />
                                            </button>
                                        </div>
                                    </div>
                                </Transition>
                ...
                <div v-else-if="sessionState === 'complete'"
                    class="p-8 md:p-16 flex flex-col items-center justify-center text-center gap-10 animate-fade-in bg-white dark:bg-neutral-900 min-h-[500px]">
                    <div class="flex flex-col items-center gap-6">
                        ...
                        <div class="flex flex-col gap-2">
                            <span class="text-[10px] tracking-[0.4em] uppercase font-bold text-primary">Session
                                Complete</span>
                            <h2 class="text-4xl font-bold tracking-tight text-neutral-900 dark:text-white">Great job!
                            </h2>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
                        ...
                        <div
                            class="flex flex-col gap-6 p-8 rounded-4xl bg-green-50/50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/20 shadow-sm transition-all duration-500 animate-fade-in text-left">
                            <div
                                class="flex items-center gap-3 text-[10px] tracking-widest uppercase font-bold text-green-600 dark:text-green-400">
                        ...
                        <div
                            class="flex flex-col gap-6 p-8 rounded-4xl bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 shadow-sm transition-all duration-500 animate-fade-in [animation-delay:100ms] text-left">
                            <div
                                class="flex items-center gap-3 text-[10px] tracking-widest uppercase font-bold text-amber-600 dark:text-amber-400">
                        ...
                        <div
                            class="flex flex-col gap-6 p-8 rounded-4xl bg-primary-50/50 dark:bg-primary-900/10 border border-primary-100 dark:border-primary-900/20 shadow-sm transition-all duration-500 animate-fade-in [animation-delay:200ms] text-left">
                            <div
                                class="flex items-center gap-3 text-[10px] tracking-widest uppercase font-bold text-primary">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex items-end gap-4">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span class="text-2xl font-bold text-neutral-400 line-through">{{
                                            recommendedLesson?.assessment?.passProbBefore }}</span>
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span class="text-4xl font-bold text-primary">{{
                                            recommendedLesson?.assessment?.passProbAfter }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2 text-xs font-bold text-primary mt-2">
                                    <span class="bg-primary/10 px-2 py-1 rounded-full">+{{
                                        parseInt(recommendedLesson?.assessment?.passProbAfter || '0') -
                                        parseInt(recommendedLesson?.assessment?.passProbBefore || '0') }}% pass
                                        probability</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        class="flex flex-col items-center gap-6 w-full max-w-2xl px-6 py-10 rounded-[3rem] bg-primary-50/30 dark:bg-primary-900/5 border border-primary-100/50 dark:border-primary-900/10 animate-fade-in [animation-delay:400ms] relative">
                        ...
                        <p
                            class="text-xl md:text-2xl leading-relaxed text-neutral-800 dark:text-neutral-200 italic relative z-10 px-4">
                        ...
                        <div class="flex items-center gap-3 relative z-10 pt-2">
                            ...
                            <span class="text-[10px] tracking-[0.2em] font-bold uppercase text-neutral-400">Acumen
                                AI</span>
                            <div class="w-8 h-px bg-primary/30"></div>
                        </div>
                    </div>
                    ...
                    <div>
                        <UButton label="See Your Readiness Plan" trailing-icon="i-lucide-arrow-right" size="xl"
                            color="primary" variant="solid" @click="sessionState = 'idle'"
