...
<template>
    <ContentPanel v-if="topic" :title="topic.title" back-link="/app/dashboard">
        <template #toolbar>
            <div class="flex items-center gap-2">
                <UBadge :label="topic.tag" variant="soft" />
                <UBadge v-if="topic.learningGoal" :label="topic.learningGoal" variant="subtle" color="primary" />
            </div>
        </template>

        <div class="w-full lg:max-w-4xl mx-auto flex flex-col gap-8 pb-12">
            <!-- Hero Section -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-2 flex flex-col gap-6">
                    ...
                    <div
                        class="flex items-center justify-between border-b border-neutral-200 dark:border-neutral-800 pb-4">
                        <h2 class="text-lg font-bold flex items-center gap-2">
                            <UIcon name="i-lucide-list-ordered" class="text-primary" />
                            Lesson Timeline
                        </h2>
                        <span
                            class="text-xs font-medium text-dimmed uppercase tracking-wider bg-neutral-100 dark:bg-neutral-800 px-2 py-1 rounded-md">
                            Generated by AI
                        </span>
                    </div>
                    ...
                    <div class="relative pl-6 ml-2 mt-4 space-y-8">
                        <!-- Vertical Line -->
                        <div
                            class="absolute left-[-1px] top-4 bottom-4 w-0.5 bg-neutral-200 dark:bg-neutral-800 rounded-full" />

                        <div v-for="(lesson, index) in lessons" :key="index" class="relative group">
                            <!-- Timeline Dot / Icon -->
                            <div class="absolute -left-[37px] top-1.5 w-7 h-7 rounded-full flex items-center justify-center border-[3px] border-white dark:border-neutral-900 z-10 transition-colors duration-300"
                                :class="{
                                    'bg-green-500 text-white': lesson.status === 'completed',
                                    'bg-primary-500 text-white shadow-[0_0_15px_rgba(var(--color-primary-500),0.5)]': lesson.status === 'current',
                            ...
                            <UCard variant="subtle" class="transition-all duration-300 transform" :class="{
                                'opacity-70 grayscale-[0.3] hover:grayscale-0 hover:opacity-100 cursor-pointer': lesson.status === 'completed',
                                'ring-2 ring-primary-500 shadow-md transform -translate-y-1 relative overflow-hidden': lesson.status === 'current',
                                'opacity-50 pointer-events-none': lesson.status === 'locked',
                                'hover:-translate-y-1 hover:shadow-md cursor-pointer': lesson.status !== 'locked' && lesson.status !== 'current'
                            }" :ui="{ body: 'flex items-start gap-4 p-4 sm:p-5' }">

                                <div v-if="lesson.status === 'current'"
                                ...
                                <div class="bg-neutral-100 dark:bg-neutral-800 p-2.5 rounded-lg shrink-0"
                                    :class="{ 'text-primary-500 dark:text-primary-400 bg-primary-50 dark:bg-primary-500/10': lesson.status === 'current' }">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between gap-2">
                                        <h3 class="font-bold text-base truncate"
                                            :class="{ 'text-primary-600 dark:text-primary-400': lesson.status === 'current' }">
                                            {{ lesson.title }}
                                        </h3>
                                    ...
                                    <p class="text-sm text-dimmed mt-1.5 leading-relaxed">{{ lesson.summary }}</p>

                                    <div class="flex items-center gap-3 mt-4" v-if="lesson.status !== 'locked'">
                                        <UButton v-if="lesson.status === 'current'" label="Start Lesson"
                                            icon="i-lucide-play-circle" color="primary" size="sm" :to="lesson.type === 'quiz' ? `/app/assessment/${lesson.id}` : `/app/lesson/${lesson.id}`" />
                                        <UButton v-else label="Review" icon="i-lucide-rotate-ccw" color="neutral"
                                            variant="ghost" size="sm" :to="lesson.type === 'quiz' ? `/app/assessment/${lesson.id}` : `/app/lesson/${lesson.id}`" />
                                    </div>
                                </div>
                            </UCard>
                        </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <UCard variant="soft" :ui="{ body: 'p-5 flex flex-col gap-4' }">
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-semibold uppercase tracking-wider text-muted">Overall
                                    Progress</span>
                                <span class="text-sm font-bold text-primary">{{ topic.progress }}%</span>
                            </div>
                            <UProgress :model-value="topic.progress" color="primary" size="sm" />
                        </div>
                        <p class="text-xs text-dimmed">You are doing great! Keep up the momentum to reach your goal.</p>
                    </UCard>
                    ...
                    <UCard variant="soft" :ui="{ body: 'p-5 flex flex-col gap-3' }"
                        ...
                        <p class="text-xs text-dimmed mb-2">This syllabus was dynamically generated from your uploaded
                            materials.</p>

                        <div class="flex flex-col gap-2">
                            <div
                                class="flex items-center gap-2 text-xs font-medium p-2 rounded-md bg-white dark:bg-neutral-900 shadow-sm border border-neutral-100 dark:border-neutral-800">
                                <UIcon name="i-lucide-file-text" class="text-red-500 shrink-0" />
                                <span class="truncate">{{ topic.title }} Chapter 1-3.pdf</span>
                            </div>
                            <div
                                class="flex items-center gap-2 text-xs font-medium p-2 rounded-md bg-white dark:bg-neutral-900 shadow-sm border border-neutral-100 dark:border-neutral-800">
                                <UIcon name="i-lucide-presentation" class="text-orange-500 shrink-0" />
                                <span class="truncate">Lecture_Deck_Final.pptx</span>
                            </div>
                        </div>
                    </UCard>
                </div>
    ...
    <!-- 404 Empty State -->
    <ContentPanel v-else title="Topic Not Found" back-link="/app/dashboard">
        <div class="flex flex-col items-center justify-center py-20 text-center">
            <UIcon name="i-lucide-file-question" class="text-6xl text-dimmed mb-4" />
