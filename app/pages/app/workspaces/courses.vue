    ...
    <UContainer class="max-w-4xl py-12">
        <div v-if="workspace" class="flex flex-col gap-10">
            ...
            <nav class="flex items-center gap-2 text-sm text-neutral-500 mb-2">
                ...
                <UIcon name="i-lucide-chevron-right" class="w-3.5 h-3.5" />
                <span class="font-medium text-neutral-900 dark:text-neutral-100">Workspace Courses</span>
            </nav>
            ...
            <header class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col gap-1">
                        <h1 class="text-3xl font-bold tracking-tight">Workspace Courses</h1>
                        <p class="text-neutral-500">Manage all learning modules curated for {{ workspace.name }}.</p>
                    </div>
                </div>
                <UButton label="Create Course" icon="i-lucide-plus" color="primary" to="/app/courses/new" />
            </header>

            <!-- Workspace Courses List -->
            <div class="flex flex-col gap-4">
                <div class="flex items-center justify-between px-2">
                    <h2 class="text-xs font-bold text-neutral-400 uppercase tracking-[0.2em]">
                        All Courses ({{ workspaceCourses.length }})
                    </h2>
                </div>
                ...
                <UCard
                        ...
                        <div v-for="course in workspaceCourses" :key="course.id"
                            ...
                            <div class="flex items-center gap-4">
                                <div
                                    ...
                                    <UIcon :name="course.icon" class="text-2xl" />
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-base font-bold text-neutral-900 dark:text-neutral-100 group-hover:text-primary transition-colors">{{ course.title }}</span>
                                    <div class="flex items-center gap-3 text-sm text-neutral-500 mt-1">
                                        <div class="flex items-center gap-2">
                                            <UAvatar v-if="course.createdBy?.avatar" :src="course.createdBy.avatar"
                                                size="xs" class="ring-1 ring-primary-500/20" />
                                            <span class="font-medium text-neutral-700 dark:text-neutral-300">{{ course.createdBy?.name || 'Unknown Author' }}</span>
                                        </div>
                                        <span class="text-neutral-300 dark:text-neutral-700">•</span>
                                        <span class="bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider">{{ course.tag }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center gap-6">
                                <div class="hidden sm:flex flex-col items-end">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        <span class="text-xs font-bold">{{ course.viewersCount || 0 }}</span>
                                    </div>
                                    <span class="text-[10px] text-neutral-400 uppercase font-bold">Viewing</span>
                                </div>
                                <div class="flex flex-col items-end min-w-[80px]">
                                    <span class="text-sm font-bold text-primary-500">{{ course.progress }}%</span>
                                    <div class="w-16 h-1 bg-neutral-100 dark:bg-neutral-800 rounded-full mt-1 overflow-hidden">
                                        <div class="h-full bg-primary-500" :style="{ width: `${course.progress}%` }" />
                                    </div>
                                </div>
                    ...
                    <div v-if="workspaceCourses.length === 0" class="p-12 flex flex-col items-center justify-center text-center">
                        <UIcon name="i-lucide-book-dashed" class="text-5xl text-neutral-300 mb-4" />
                        <h3 class="text-lg font-bold">No courses yet</h3>
                        <p class="text-neutral-500 max-w-xs mt-1">This workspace doesn't have any curated courses yet. Start by creating one!</p>
                        <UButton label="Create First Course" icon="i-lucide-plus" color="primary" class="mt-6" to="/app/courses/new" />
                    </div>
                </UCard>
            </div>
        </div>
