  ...
  <div class="flex flex-col gap-8">
    <div class="flex items-center gap-4">
      <div class="p-3 bg-primary-100 dark:bg-primary-900/40 rounded-xl">
        <UIcon name="i-lucide-clock" class="text-2xl text-primary-500" />
      </div>
      <div>
        <h2 class="text-xl font-bold">Schedule & Duration</h2>
        <p class="text-sm text-neutral-500">How much time can you realistically commit?</p>
      </div>
    </div>
      ...
      <UFormField label="How long do you want this topic to last?">
        <div class="flex flex-col gap-3 mt-4">
          <UCard 
            ...
            @click="updateField('duration', d.value)"
          >
            <UIcon :name="d.icon" class="text-lg" :class="modelValue.duration === d.value ? 'text-primary-500' : 'text-neutral-400'" />
            <span class="text-sm font-medium" :class="modelValue.duration === d.value ? 'text-primary-700 dark:text-primary-300' : 'text-neutral-600 dark:text-neutral-400'">{{ d.label }}</span>
      ...
      <UFormField label="What is your weekly availability?">
        <div class="flex flex-col gap-3 mt-4">
          <UCard 
            ...
            :ui="{ body: 'p-3 flex items-center gap-3' }"
            @click="updateField('availability', a.value)"
          >
            <UIcon :name="a.icon" class="text-lg" :class="modelValue.availability === a.value ? 'text-primary-500' : 'text-neutral-400'" />
            <span class="text-sm font-medium" :class="modelValue.availability === a.value ? 'text-primary-700 dark:text-primary-300' : 'text-neutral-600 dark:text-neutral-400'">{{ a.label }}</span>
