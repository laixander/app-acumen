...
<template>
  <div class="flex flex-col items-center justify-center min-h-[500px] text-center gap-10 py-8">
    <!-- Top Spinner -->
    <div class="relative">
      <div class="absolute inset-0 bg-primary-500/10 blur-3xl rounded-full scale-110" />
      <div
        class="relative w-24 h-24 bg-white dark:bg-neutral-900 shadow-xl rounded-3xl flex items-center justify-center">
        <UIcon name="i-lucide-sparkles" class="text-4xl text-primary-500 animate-sparkle-pulse" />
      </div>
    </div>
    ...
    <div class="w-full max-w-sm flex flex-col gap-3">
      <TransitionGroup name="list">
        <UCard v-for="(step, index) in steps" :key="step.id" class="border-none shadow-sm transition-all duration-300"
          :class="[
          ...
          <!-- Status Icon -->
          <div class="w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300" :class="[
            ...
            <UIcon v-else-if="step.status === 'in-progress'" name="i-lucide-loader-2" class="w-5 h-5 animate-spin" />
...
@keyframes sparkle-pulse {
  ...
  100% {
    transform: scale(1);
    opacity: 0.3;
    filter: drop-shadow(0 0 0px rgba(var(--color-primary-500), 0));
  }

  50% {
    transform: scale(1.2);
    opacity: 1;
    filter: drop-shadow(0 0 15px rgba(var(--color-primary-500), 0.5));
  }
}
