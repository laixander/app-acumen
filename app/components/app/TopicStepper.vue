...
<template>
  <div class="w-full flex items-center justify-between mb-12 relative px-4 sm:px-12">
    <!-- Connecting Lines -->
    <div class="absolute top-1/2 left-0 right-0 flex -translate-y-1/2 px-4 sm:px-12 z-0">
      <div class="flex-1 relative h-0.5 mx-5">
    ...
    <!-- Step Circles -->
    <div v-for="(step, index) in steps" :key="index" class="relative z-10 flex flex-col items-center gap-3 group">
      <div class="w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300" :class="[
        index <= currentStep
        ...
        <UIcon v-if="index < currentStep" name="i-lucide-check" class="text-xl" />
